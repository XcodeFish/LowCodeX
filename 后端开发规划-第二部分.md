...
export class DynamicEntityService {
   ...
   * 根据表名和ID查询单条记录
   */
  async findOne(tableName: string, id: string, user: User) {
    // 获取表元数据
    const metaTable = await this.metaTableService.findByName(tableName, user);
    if (!metaTable) {
      throw new NotFoundException(`表 ${tableName} 不存在`);
   ...
   * 根据表名和ID更新记录
   */
  async update(tableName: string, id: string, data: any, user: User) {
    // 获取表元数据
    const metaTable = await this.metaTableService.findByName(tableName, user);
    if (!metaTable) {
      throw new NotFoundException(`表 ${tableName} 不存在`);
   ...
   * 根据表名和ID删除记录
   */
  async remove(tableName: string, id: string, user: User) {
    // 获取表元数据
    const metaTable = await this.metaTableService.findByName(tableName, user);
    if (!metaTable) {
      throw new NotFoundException(`表 ${tableName} 不存在`);
